...
import type { BagForm } from "@/lib/types";
import Link from "next/link";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "../ui/tabs";

interface BoxSelectorProps {
  boxTypes: BagForm[];
  onSelectBox: (box: BagForm) => void;
}
...
export default function BoxSelector({ boxTypes, onSelectBox }: BoxSelectorProps) {
  ...
  //const categories = ['fruit', 'vegetable', 'mixed'] as const;

  // const renderBoxCard = (box: BoxType, index: number) => (
  //   <Card
  //     key={box.id}
        ...
        {['fruit', 'vegetable', 'mixed'].map((category) => (
          <TabsContent key={category} value={category} className="mt-6">
            <div className="grid lg:grid-cols-3 gap-8 mb-12">
              {boxTypes.filter((box) => box.category === category)
                .map((box, index) => (
                  <Card
                    key={box.id}
                    className={`relative overflow-hidden hover:shadow-xl transition-all group cursor-pointer ${getBgTintClass(index)} ${getBorderClass(index)}`}
